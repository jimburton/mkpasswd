...
{-| Substitute `c' by looking it up in the lookup table.
If `c' is not present in the lookup table, return `c'. -} 
subst :: Char -> Char
...
{-| Retrieve the option containing the path to the dictionary file from
the flag supplied, or the default value. -}
getFp :: [Flag] -> FilePath
getFp = foldl (\acc x -> case x of 
                          (WordsFile fp) -> fp
                          _              -> acc) defaultWords 

{-| Capitalise the input, `c', which is an alphabetic char, about half
  of the time. -} 

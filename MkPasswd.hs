...
The default dictionary file, /etc/dictionaries-common/words, is correct on Debian-based systems
(including Ubuntu). It isn't tested on Windows but if you supply the location of a file that contains
a list of words, one per line, then that should work.
...
subst :: Char -> Char
subst c = case lookup (toLower c) subst_table of
            (Just c') -> c'
            Nothing   -> c
...
maybeUpper c = do f <- randFloat
                  let i = round f
                  if i == 0 then return $ toUpper c else return c

{-| Create a password according to the flags supplied. -}                  
mkPasswd :: [Flag] -> IO String
mkPasswd fs = 
    do let h  = elem Help fs
           w  = elem Wordy fs
           v  = elem Version fs
           s  = elem Strong fs
           x  = elem VeryStrong fs
           e  = elem Explain fs
           l  = getLen fs
    ...
    do str <- readFile fp
       ...
       if null ls 
        then error ("There are no words which are "++(show n)++" letters long in the file "++fp)
                ...
                let w = ls!!i
                w' <- sequence $ map (maybeUpper . subst) w
                if e then return (w' ++ " [" ++ w ++ "]") else return w' 
...
processFlags fs | elem Help fs    = putStrLn $ usageInfo header options
                | elem Version fs = putStrLn version
